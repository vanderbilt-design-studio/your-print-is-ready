...
make clean
cmake -DOPENSSL_ROOT_DIR="$OPENSSL_ROOT_DIR" -DOPENSSL_INCLUDE_DIR="$OPENSSL_INCLUDE_DIR" -DOPENSSL_CRYPTO_LIBRARY="$OPENSSL_CRYPTO_LIBRARY" -DOPENSSL_SSL_LIBRARY="$OPENSSL_SSL_LIBRARY" -DCMAKE_TOOLCHAIN_FILE=../../cmake/toolchain/toolchain-rpi-c.cmake . && make -j$(nproc)
cd -